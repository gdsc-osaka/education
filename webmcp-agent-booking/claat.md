...
![運営用管理フロントのログイン後の画面。60席のグリッドと予約一覧が表示されている](img/step2-admin-dashboard.svg)
...
![開発者ツールのConsoleでlistTools()を実行し、3件のツールが表示されている様子](img/step3-devtools-listtools.svg)
...
![coordinator が場所・値段・効果の3specialistとreservation specialistをA2A経由で呼び出す全体構成図](img/step5-agent-architecture.svg)
...
![coordinatorのWorkflowが希望解析からリトライ付き予約まで進む処理順序を番号付き矢印で示した図](img/step5-workflow-sequence.svg)
...
![ADK Web UIでのチャット応答と、予約サイト側の予約一覧に反映された様子](img/step9-integration-result.svg)
